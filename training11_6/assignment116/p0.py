...
prices = {1: 1.00, 2: 1.50, 3: 2.00}

def payloop():
    totalpaid = 0
    for i in coins:
        payment = int(input(f"How many {i} will you insert?\n")) * coins[i]
        # print(payment)
        totalpaid += payment
    return totalpaid

def changeloop(change: int):
    charr = {}
    inchange = change
    for i in coins:
        charr[i] = 0
...
if __name__ == "__main__":
    vendingmsg = "Welcome to my vending machine!\n1: Water: $1:00\n2: Cola: $1:50\n3: Gatorade: $2:00"
    ...
    while totalpaid < price:
        print("Not enough money was entered")
        totalpaid += payloop()
    totalpaid = round(totalpaid - price, 2)
